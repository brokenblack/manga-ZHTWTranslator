...
| 檔案 | 大小 | 啟動 | 說明 |
|---|---|---|---|
| [`翻譯工具.exe`](https://github.com/brokenblack/manga-TCNtranslator/releases/download/v1.1.0/翻譯工具.exe) (v1.1.0) ⭐ | 402 MB | 10-15 秒 | **最新版單檔，下載即用**，新增 `Ctrl+Alt+W` 喚回視窗熱鍵 |
...
2. 雙擊 `翻譯工具.exe`（單檔版首次啟動需 10-15 秒解壓，請耐心等候）
3. 點右上角 **⚙️ 翻譯設定** → **翻譯 API** 分頁
4. 申請並填入 [API Key] → 儲存
5. 按 **Ctrl + Alt + Q** 框選遊戲文字 → 自動翻譯！

> 💡 程式會在 exe 同層目錄自動生成 `config.json`（設定）與 `history.json`（翻譯紀錄），所以建議放在固定資料夾而非下載區。

完整操作說明與常見問題請見 Release 頁面一併提供的 **使用說明.txt**。
...
每次發布都會通過 VirusTotal 掃描，幾個大廠（Microsoft / Avira / CrowdStrike 等）皆 clean。各版本的掃描報告連結附在對應的 Release 頁面。

⚠️ 由於是個人開發工具未經商業簽章，Windows Defender 可能有誤報。處理方式請見 Release 頁面一併提供的 [**使用說明.txt**](https://github.com/brokenblack/manga-TCNtranslator/releases/download/v1.1.0/使用說明.txt) 之常見問題。
